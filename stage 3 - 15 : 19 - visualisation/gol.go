...
func buildWorkerWorld(world [][]byte, actualHeight, imageHeight, imageWidth, currentThreads int) [][] byte{
	workerWorld := make([][]byte, actualHeight + 2)
	for j := range workerWorld {
		workerWorld[j] = make([]byte, imageWidth)
	}

	for x := 0; x < imageWidth; x++ {
			workerWorld[0][x]=world[(currentThreads * actualHeight + imageHeight - 1) % imageHeight][x]
	}

	for y := 1; y <= actualHeight; y++ {
		for x := 0; x < imageWidth; x++ {
			workerWorld[y][x]=world[currentThreads * actualHeight + y - 1][x]
		}
	}

	for x := 0; x < imageWidth; x++ {
			workerWorld[actualHeight+1][x]=world[((currentThreads+1)*actualHeight + imageHeight)%imageHeight][x]
	}
	visualiseMatrix(world,imageWidth,imageHeight)
	return workerWorld
}
